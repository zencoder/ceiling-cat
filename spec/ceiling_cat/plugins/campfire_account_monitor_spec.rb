...
describe "Campfire Account Monitor" do
  before(:each) do
    ...
    @connection = CeilingCat::Campfire::Connection.new(OpenStruct.new({:service => 'campfire', :username => user, :token => token, :room => 'Room 1', :plugins => plugins, :max_users => @user_limit}))
